...
const MembershipSystem = artifacts.require("EsimMembership.sol");
...
contract("MembershipSystem", (addresses) => {
  ...
  it("creates plans", async () => {
    await membershipSystem.createMembershipPlan(token.address, 25, ONE_WEEK, {
      from: merchant,
    });
    await membershipSystem.createMembershipPlan(
      token.address,
      100,
      THIRTY_DAYS,
      { from: merchant }
  ...
  it("fails to create a plan", async () => {
    await expectRevert(
      membershipSystem.createMembershipPlan(
    ...
    await expectRevert(
      membershipSystem.createMembershipPlan(token.address, 0, THIRTY_DAYS, {
        from: merchant,
      }),
      "membership amount is 0"
    );
    await expectRevert(
      membershipSystem.createMembershipPlan(token.address, 100, 0, {
        from: merchant,
      }),
  ...
  it("attempt to subscribe to a non existent plan", async () => {
    await expectRevert(
      membershipSystem.subscribe(0, { from: subscriber }),
  ...
  it("attempt to renew a membership before the due date ", async () => {
    ...
    await expectRevert(
      membershipSystem.renew(subscriber, 0),
  ...
  it("cancellation of a membership", async () => {
    ...
    assert(
      (await membershipSystem.memberships(subscriber, 0).length) === undefined
    );
  });
});
